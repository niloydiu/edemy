    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 py-12 w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          {/* Analytics Panel */}
          {analytics && (
            <div className="glassmorphic p-6 rounded-lg border border-white/10 space-y-4 shadow-neonPurple/5 shadow-lg">
              <h3 className="text-sm font-black uppercase tracking-wider text-neonPurple flex items-center gap-1.5 border-b border-white/5 pb-2">
                <TrendingUp className="w-4 h-4" /> Neural Analytics
              </h3>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/5 p-3 rounded border border-white/5">
                  <div className="text-[9px] text-gray-500 font-bold uppercase">Students</div>
                  <div className="text-lg font-black text-neonCyan mt-0.5">{analytics.totalStudents}</div>
                </div>
                <div className="bg-white/5 p-3 rounded border border-white/5">
                  <div className="text-[9px] text-gray-500 font-bold uppercase">Earnings</div>
                  <div className="text-lg font-black text-neonCyan mt-0.5">${analytics.totalEarnings.toFixed(2)}</div>
                </div>
              </div>
              ...
              <div className="space-y-2 pt-2 border-t border-white/5">
                ...
                <div className="space-y-1.5">
                  {analytics.ratingDistribution?.map((val: number, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center justify-between text-[9px] text-gray-500">
                        <span className="w-6">{stars}★</span>
                        <div className="flex-1 mx-2 h-1.5 bg-gray-800 rounded-full overflow-hidden">
                          <div className="h-full bg-neonPurple" style={{ width: `${percent}%` }} />
                        </div>
          ...
          <div className="glassmorphic p-6 rounded-lg border border-white/10">
            ...
            {loading ? (
              <div className="text-xs text-gray-500 animate-pulse font-bold uppercase tracking-widest">
                Fetching Schedule Matrix...
              </div>
            ) : schedule.length === 0 ? (
              <p className="text-xs text-gray-500 uppercase tracking-widest font-bold">
                No active online/offline classes scheduled.
              </p>
            ) : (
              <div className="space-y-4">
                {schedule.map((item, idx) => (
                  <div key={idx} className="bg-white/5 border border-white/10 p-4 rounded space-y-2">
                    <div className="flex justify-between items-start">
                      <span className={`text-[9px] px-2 py-0.5 rounded font-black uppercase tracking-wider ${
                        item.lessonType === 'online' ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                      }`}>
                        {item.lessonType} Class
                      </span>
                      <span className="text-[10px] text-gray-500 font-bold">
                        <Clock className="w-3.5 h-3.5 inline mr-1 text-neonCyan" />
                        {item.duration}m
                      </span>
                    </div>
                    <h4 className="text-xs font-black uppercase text-white leading-snug">
                      {item.lessonTitle}
                    </h4>
                    <p className="text-[10px] text-gray-400 truncate">Course: {item.courseTitle}</p>
                    ...
                    {item.lessonType === 'online' && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-[9px] text-neonCyan underline font-black flex items-center gap-1.5"
                      >
                        <Video className="w-3.5 h-3.5" /> Launch Meet Room
                      </a>
                    )}
                    {item.lessonType === 'offline' && (
                      <p className="text-[9px] text-emerald-400 font-bold flex items-center gap-1.5">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="flex justify-between items-center border-b border-white/5 pb-4">
            <div>
              ...
              <p className="text-xs text-gray-500 uppercase tracking-wide">
                Publish and modify syllabus nodes
              </p>
            </div>
            <button
              onClick={() => setShowCourseForm(!showCourseForm)}
              className="px-4 py-2 bg-neonCyan text-black text-xs font-black rounded uppercase tracking-widest flex items-center gap-1.5 cursor-pointer"
            >
          ...
          {showCourseForm && (
            <motion.form
              ...
              className="glassmorphic p-6 rounded-lg border border-neonCyan/30 space-y-4"
            >
              <h4 className="text-sm font-black uppercase text-neonCyan">Initialize New Course Node</h4>
              <div>
          ...
          {loading ? (
            <div className="text-xs text-gray-500 animate-pulse font-bold uppercase tracking-widest py-12">
              Scanning Published blue prints...
            </div>
          ) : courses.length === 0 ? (
            <div className="glassmorphic p-8 text-center rounded border border-white/5">
              <p className="text-xs text-gray-500 uppercase tracking-widest font-bold">
                You have not deployed any courses yet.
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {courses.map((course) => (
                <div key={course._id} className="glassmorphic p-6 rounded-lg border border-white/10 space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                    ...
                    <button
                      ...
                      className="px-3 py-1 bg-neonCyan/10 hover:bg-neonCyan text-neonCyan hover:text-black border border-neonCyan/20 rounded text-xs font-black uppercase tracking-widest transition-all cursor-pointer"
                    >
                      + Add Lesson
                    </button>
                  </div>
                  ...
                  <div className="pl-4 border-l border-white/5 space-y-2">
                    ...
                    {course.lessons && course.lessons.length > 0 ? (
                      course.lessons.map((lesson: any, idx: number) => (
                        <div key={lesson.lessonId} className="flex justify-between items-center text-xs text-gray-400">
                          <span>{idx + 1}. {lesson.lessonTitle}</span>
                          <span className="text-[9px] bg-white/5 border border-white/10 px-2 py-0.5 rounded text-gray-500 font-bold uppercase">
                            {lesson.lessonType}
                          </span>
                        </div>
                      ))
                    ) : (
                      <p className="text-[10px] text-gray-600 font-bold uppercase">No lessons added to this course.</p>
                    )}
                  </div>
                  ...
                  {course.enrolledStudents && course.enrolledStudents.length > 0 && (
                    <div className="pt-4 border-t border-white/5 space-y-2">
                      <h5 className="text-[10px] font-black uppercase tracking-wider text-gray-500 flex items-center gap-1">
                        <Users className="w-3.5 h-3.5 text-neonPurple" /> Active Enrolled student list
                      </h5>
                      <div className="flex flex-wrap gap-2">
                        {course.enrolledStudents.map((sid: string) => (
                          <div key={sid} className="flex items-center gap-2 bg-white/5 p-1 px-2.5 rounded border border-white/10">
                            <span className="text-[10px] font-bold text-gray-300">ID: {sid}</span>
                            <Link href={`/chat?recipientId=${sid}`} className="text-neonCyan hover:text-white transition-all">
          ...
          {showLessonForm && activeCourseId && (
            <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-6">
              <motion.form
                ...
                className="w-full max-w-lg glassmorphic p-8 rounded-lg border border-white/15 space-y-4"
              >
                <div className="flex justify-between items-center">
                  <h4 className="text-sm font-black uppercase text-neonCyan">Add Lesson to Course</h4>
                  <button
                    type="button"
                ...
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full mt-4 py-3 bg-neonCyan text-black font-extrabold uppercase text-xs rounded hover:scale-[1.02] transition-all cursor-pointer"
                >
