...
import { BookOpen, Calendar, MapPin, Video, FileText, Star, CreditCard, ChevronRight, Lock, CheckSquare, Square, Play } from 'lucide-react';
...
export default function CourseDetailsPage() {
  ...
  const loadCourseAndProgress = useCallback(async () => {
    try {
      ...
      const isEnr = user?.enrolledCourses?.includes(String(res.data.id)) || user?.enrolledCourses?.includes(String(res.data._id));
      if (isEnr && user) {
        const pRes = await api.get(`/courses/${res.data.id || res.data._id}/progress`);
        setProgress(pRes.data);
        
        // Auto-select first lesson with video/content if available
    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 py-12 w-full grid grid-cols-1 lg:grid-cols-3 gap-12 pt-24">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* Active Lesson Player for Enrolled Users */}
          ...
          >
            ...
            <div className="flex items-center gap-6 pt-2">
              ...
              <span className="text-slate-500 text-xs font-bold uppercase tracking-wider">
                Instructors: {course.tutorNames || 'Professor Tech'}
              </span>
            </div>
          </motion.div>
                  ...
                  return (
                    <div
                      ...
                      className={`p-5 rounded-lg flex items-start gap-4 border transition-all shadow-sm ${
                        ...
                        isSelected ? 'border-indigo-600 bg-indigo-50/20 hover:bg-indigo-50/30' : 'bg-white border-slate-200'
                      }`}
                    >
                      <div className="w-8 h-8 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center text-xs font-bold text-slate-600 flex-shrink-0">
                        {index + 1}
                      </div>
                      ...
                      <div className="flex-1 min-w-0 space-y-1.5">
                        <div className="flex items-center justify-between gap-4">
                          ...
                          <span className="text-[10px] bg-slate-50 border border-slate-200 px-2 py-0.5 rounded text-slate-600 font-bold uppercase tracking-wider flex-shrink-0">
                            {lesson.lessonType}
                          </span>
                        </div>
                        ...
                        {lesson.lessonType === 'video' && (
                          <p className="text-xs text-slate-500 flex items-center gap-1.5">
                            <Play className="w-3.5 h-3.5 text-indigo-500" /> Video Lecture File
                          </p>
                        )}
                      ...
                      {isEnrolled && (
                        <button
                          ...
                          className={`ml-4 p-1 rounded hover:bg-slate-100 flex-shrink-0 cursor-pointer ${
          ...
          >
            ...
            {isEnrolled && progress && (
              <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-lg space-y-2">
                <div className="flex justify-between text-xs font-bold text-indigo-950 uppercase">
            ...
            <div className="space-y-2">
              <div className="text-3xl font-extrabold text-slate-900">${Number(course.coursePrice) || 0}</div>
            ...
            {isEnrolled ? (
              <div className="space-y-2">
                <span className="w-full py-3 bg-emerald-50 text-emerald-700 font-extrabold uppercase text-center text-xs rounded border border-emerald-200 flex items-center justify-center gap-2 shadow-sm">
                  Enrolled & Active
                </span>
                <button
